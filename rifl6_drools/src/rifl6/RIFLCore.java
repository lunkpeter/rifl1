					...
					try {
						...
						if("quit".equals(input)) {
							...
							for (String param : params) {
								if("0".equals(param)){
									orders.add(craetePostalOrder());
									command = Command.send;
								} else if("1".equals(param)){
									orders.add(craetePrivateOrder());
									command = Command.send;
